...
export class HeaderComponent implements OnInit, OnDestroy {
    ...
    userRoleId : any; userData: any = [];

    eventSubscriptions: Subscription[] = [];

	constructor( private _http: HttpService, private _user: UserService, private _router: Router, private _socialAuthService: AuthService, private _alert: AlertService, private _event: EventService, private _helper: HelperService ) { 
    ...
    logout() {
        ...
        .subscribe(
            (res) => {
                ...
                let data = [];
				this._event.fire('setRoleId', data);
		...
		if($(".navbar-toggler").is(":visible")) {
		   ...
		   this.isCollapsed = !this.isCollapsed;
			if(this.isCollapsed==true){
				...
				setTimeout(function(){
					$('#inputSearch').focus();
					var placeholder = 'pmka';
					var index = 0;
					var interval = setInterval(function(){
						var p = placeholder.charAt(index);
						var ph = '';
						if($('#inputSearch').attr('placeholder')){
							ph = $('#inputSearch').attr('placeholder');
						}
						$('#inputSearch').attr('placeholder', (ph+p));
						index++;
						if((ph.length+1)==placeholder.length){
